...
def generate_citizens(num_records):
    data = {
        'CitizenID': range(1, num_records + 1),
        'Name': [fake.name() for _ in range(num_records)],
        'Age': [random.randint(18, 80) for _ in range(num_records)],
        'AddressID': [random.randint(1, num_records) for _ in range(num_records)],
        'TaxID': [random.randint(1, num_records) for _ in range(num_records)]
    }
...
def generate_addresses(num_records):
    data = {
        'AddressID': range(1, num_records + 1),
...
if __name__ == "__main__":
    ...
    path = "C:/Users/quindata/OneDrive/Bureau/government/"
